...
import { approvePass, approveReject, getComponentsData, getList, updateContent } from './api';
...
const ContentList: React.FC = () => {
  ...
  const EditChange = async (item: any) => {
    console.log('编辑文件名：', item);
    const param = {
      contentId: item.contentId,
      fileName: item.fileName,
    };
    const res = await updateContent(param);
    if (res.code == 200) {
